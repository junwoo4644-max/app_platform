...
├── leaf_area/
│   ├── backend/              FastAPI (:8000, 내부 전용)
│   └── frontend/             React/Vite → nginx (:80, 내부 전용)
├── lot platform/             별도 서버에 독립 배포되는 앱 (변경 없음)
├── nginx/nginx.conf          서버 1의 진입점 (portal, leaf_area로 라우팅)
└── docker-compose.yml        서버 1 전체 스택 (portal + leaf_area + nginx)
```
...
- **서버 1**: `portal` + `leaf_area` (이 저장소의 `docker-compose.yml` 하나로 배포)
...
- leaf_area: `http://<서버1 IP>:8081/` (nginx가 leaf-area-frontend 컨테이너로 프록시)

최초 기동 시 `.env`의 `ADMIN_EMAIL`/`ADMIN_PASSWORD`로 로그인 가능한 관리자 계정이 자동 생성됩니다.
로그인 후 `/admin`에서 leaf_area를 앱으로 등록하세요 (URL: `http://<서버1 IP>:8081`, 썸네일 업로드 가능).
lot_platform도 같은 방식으로 `http://<서버2 IP>:3000`을 등록하면 포털 목록에 함께 나타납니다.

## 나중에 새 앱을 추가하는 방법 (서버 1에 함께 둘 경우)

1. 새 앱을 `app_platform/<new-app>/`에 도커화해서 넣는다 (leaf_area처럼 Dockerfile 보유).
2. `docker-compose.yml`에 서비스 블록 추가 (호스트 포트는 노출하지 않음, leaf-area-frontend 참고).
...
- 앱들(leaf_area, lot_platform) 코드는 그대로 둡니다.
